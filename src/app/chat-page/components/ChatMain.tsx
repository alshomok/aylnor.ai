...
import { Zap, Brain, Code2, Send, Paperclip, PanelRight, PanelRightClose, Menu, Copy, Check, RotateCcw, ThumbsUp, ThumbsDown } from 'lucide-react';
...
export default function ChatMain({
  ...
  const handleSend = async () => {
    ...
    const responseData = MOCK_RESPONSES[activeMode](content);
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`flex items-center gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all duration-150 ${
                showCodePanel
                  ? 'badge-blue border-royal-blue/30' :'text-muted-foreground border-border hover:border-royal-blue/30 hover:text-foreground'
              }`}
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin px-3 sm:px-4 py-6 space-y-5">
          {messages.map((msg) => (
            ...
            >
              ...
              <div className={`max-w-[85%] sm:max-w-xl group ${msg.role === 'user' ? 'items-start' : 'items-end'} flex flex-col gap-1`}>
                {/* Mode badge for bot */}
                {msg.role === 'bot' && msg.mode && (
                  <div className="flex items-center gap-1.5 mb-0.5 flex-row-reverse">
                    ...
                    <span className={`text-2xs px-1.5 py-0.5 rounded-full font-semibold ${
                      msg.mode === 'quick' ? 'badge-gold' :
                      msg.mode === 'thoughtful' ? 'badge-blue' : 'badge-green'
                    }`}>
                ...
                >
                  <p className="whitespace-pre-wrap text-right">{renderMessageContent(msg.content)}</p>
                  ...
                  {msg.codeBlock && (
                    <div className="mt-3">
                      ...
                      <pre className="code-block bg-black/40 rounded-lg p-3 text-xs overflow-x-auto max-h-40 text-green-300 text-left" dir="ltr">
                        <code>{msg.codeBlock.code.slice(0, 300)}{msg.codeBlock.code.length > 300 ? '\n…' : ''}</code>
                      </pre>
                    </div>
                  )}
                </div>
