...
export default function CodeDisplayPanel({ codeBlock, onClose }: CodeDisplayPanelProps) {
  ...
  const handleDownload = () => {
    if (!codeBlock) return;
    const ext: Record<string, string> = { python: 'py', javascript: 'js', typescript: 'ts', java: 'java', cpp: 'cpp', sql: 'sql' };
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border flex-row-reverse">
        <div className="flex items-center gap-2.5 flex-row-reverse">
          ...
          <span className="text-sm font-bold text-foreground">
            {codeBlock ? (LANGUAGE_LABELS[codeBlock.language] ?? codeBlock.language) : 'الكود'} — المخرجات
          </span>
          {codeBlock && (
            ...
            {codeBlock ? (
              ...
              <div className="flex flex-col items-center justify-center h-full py-16 px-6 text-center">
                ...
                <p className="text-xs text-muted-foreground max-w-xs">
                  بدّل إلى وضع <span className="text-royal-blue-light font-semibold">البرمجة</span> واطلب من aylnor كتابة كود — سيظهر هنا مع تمييز الصياغة.
                </p>
              </div>
            )}
          </div>
            ...
            {codeBlock ? (
              <div>
                <div className="flex items-center gap-2 mb-3 flex-row-reverse">
                  ...
                  <span className="text-2xs text-muted-foreground">(شغّل محلياً لرؤية المخرجات الحقيقية)</span>
                </div>
                <pre className="code-block bg-black/60 rounded-xl p-4 text-xs text-green-300 leading-6 overflow-x-auto border border-green-500/15" dir="ltr">
                  {outputText}
                </pre>
                <div className="mt-4 glass-card rounded-xl p-4 border border-gold/15">
                  ...
                  <div className="space-y-1.5 text-right">
                    {codeBlock.language === 'python' && (
                      <>
                        <p className="text-xs text-muted-foreground">١. احفظ الملف باسم <code className="text-green-300 font-mono" dir="ltr">solution.py</code></p>
                        <p className="text-xs text-muted-foreground">٢. شغّل: <code className="text-green-300 font-mono" dir="ltr">python solution.py</code></p>
                        <p className="text-xs text-muted-foreground">٣. أو استخدم: <code className="text-green-300 font-mono" dir="ltr">python3 solution.py</code> على macOS/Linux</p>
                      </>
                    )}
                    {(codeBlock.language === 'javascript' || codeBlock.language === 'typescript') && (
                      <>
                        <p className="text-xs text-muted-foreground">١. احفظ الملف باسم <code className="text-green-300 font-mono" dir="ltr">solution.{codeBlock.language === 'typescript' ? 'ts' : 'js'}</code></p>
                        <p className="text-xs text-muted-foreground">٢. شغّل: <code className="text-green-300 font-mono" dir="ltr">node solution.js</code></p>
                      </>
                    )}
                    {!['python', 'javascript', 'typescript'].includes(codeBlock.language) && (
                      <p className="text-xs text-muted-foreground">
                        اسأل aylnor: <span className="text-gold italic">&quot;كيف أشغّل كود {codeBlock.language} هذا؟&quot;</span>
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center justify-center h-full py-16 text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">ولّد كوداً أولاً لرؤية المخرجات المحاكاة.</p>
              </div>
            )}
          </div>
        )}
          ...
          <div className="h-full overflow-auto scrollbar-thin p-4 space-y-4">
            <div className="mb-2 text-right">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                aylnor يولّد كوداً بجودة إنتاجية في وضع البرمجة. إليك كيفية الاستفادة القصوى من لوحة الكود.
              </p>
            </div>
            ...
            <div className="glass-card rounded-xl p-4 border border-gold/15 mt-2">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <p key={`prompt-template-${i}`} className="text-xs text-muted-foreground italic border-r-2 border-gold/30 pr-3 text-right">
                    {prompt}
                  </p>
                ))}
              </div>
            ...
            <div className="glass-card rounded-xl p-4 border border-border">
              ...
              <div className="flex flex-wrap gap-1.5 justify-end">
                {[
                  ...
                  'Go', 'Rust', 'SQL', 'Bash', 'HTML', 'CSS', 'React',
                  'Swift', 'Kotlin', 'PHP', 'Ruby', 'R', 'MATLAB',
                ].map((lang) => (
                  <span
                    key={`lang-${lang}`}
